...
class Game {
    ...
    /** Return the current manual player. */
    public Player getManual() {
        return _manualPlayerTemplate;
    }

    /** Return the auto player. */
    public Player getAuto() {
        return _autoPlayerTemplate;
    }
    ...
    public void manualCommand(String player) {
        switch (player) {
        case "white":
    ...
    public void autoCommand(String player) {
        switch (player) {
        case "white":
            _white = _autoPlayerTemplate.create(WP, this);
            break;
        ...
        while (true) {
            try {
                ...
                if (_board.gameOver() && _playing) {
                    HashMap Btest = _black.getBMap();
                    HashMap Wtest = _black.getWMap();
//                    System.out.println(Btest.size());
                    System.out.println(_board.toString());
//                    File BMap = new File("loa/BlackMap");
//                    File WMap = new File("loa/WhitekMap");
//                    HashMap<String, Double> test = new HashMap<>();
//                    test.put(_board.toString(), 1.0);
//                    Utils2.writeObject(BMap, Btest);
//                    Utils2.writeObject(WMap, Wtest);
                    if (_board.winner() == BP) {
                        return BP;
                    } else if(_board.winner() == WP) {
                        return WP;
                    } else {
                        return EMP;
                    }
