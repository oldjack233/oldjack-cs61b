...
//import static loa.JACKPlayer.WMap;
import static loa.JackMachinePlayer.BMap;
import static loa.JackMachinePlayer.BlackMap;
...
public class BoardTest {
    ...
    /** A "general" position. */
    static final Piece[][] BOARD11 = {
            { EMP, EMP,  EMP,  BP,  BP, EMP, EMP, EMP },
            { WP,  EMP, EMP, EMP, EMP, EMP, EMP, WP  },
            { WP,  BP, EMP, EMP,  BP,  BP, EMP, WP  },
            { WP,  EMP,  BP, EMP, EMP,  WP, EMP, EMP  },
            { WP,  EMP,  WP,  WP, EMP,  WP, EMP, EMP  },
            { WP,  EMP, EMP, EMP,  BP, EMP, EMP, WP  },
            { EMP, EMP, EMP, EMP, EMP, EMP, EMP, EMP  },
    ...
    public void testFindMove() {
        Board b0 = new Board(Board.BOARD1, BP);
        Board b1 = new Board(BOARD1, BP);
        Board b11 = new Board(BOARD11, BP);
        Board b2 = new Board(BOARD2, BP);
        Board b3 = new Board(BOARD3, BP);
//        double ans0 = b0.miniScore(BP);
//        List<Move> mvs = b1.efficientMoves(Square.sq(3, 0));
//        double ans = b1.miniScore(BP);
        int validMoves = 0;
        double ans2 = b11.miniScore(BP);
        for (int row = 0; row < 8; row++) {
            for (int col = 0; col < 8; col++) {
                Square sqnow = Square.sq(row, col);
                if (b0.get(sqnow) == WP) {
                    for (Move m : b3.legalMoves(sqnow)) {
                        validMoves ++;
                    }
                }
            }
        ...
        int i = 0;
        while(i < 10) {
            double score = MachinePlayer.findMove(b1, 4, true,
                -1, -100000000, 100000000);
            i++;
        }
//        HashMap<String, Double> WhiteMap = Utils2.readObject(WMap, HashMap.class);
...
//        test.put("hi", 1.0);
//        Utils2.writeObject(testFile, test);
//        Utils2.readObject(BMap, HashMap.class);
