...
public class Main {
    ...
    public static void main(String... args) {
        CommandArgs options =
            new CommandArgs("--debug=(\\d+){0,1} --display{0,1} --strict{0,1} --vs{0,1} "
                            + "--log={0,1} --=(.*){0,2}",
                            args);
...
//                System.out.println(game.getBoard().toString());
            ...
            System.out.println("Run " + GameNum + " games  against two AI");
            System.out.println("Black(Jack) win rate:" + bp);
            System.out.println("White win rate: " + wp);
            System.out.println("Tie rate: " + tie);
    ...
    private static Game getGame(CommandArgs options) {
        ...
        else if (options.contains("--vs")) {
            gui = null;
            reporter = new TextReporter();
            view = new NullView();
            manualPlayer = new MachinePlayer();//黑
            machinePlayer = new MachinePlayer();//白
        }
        else {
            gui = null;
            reporter = new TextReporter();
        ...
        return new Game(view, log, reporter, manualPlayer,
                machinePlayer, options.contains("--strict"));
    }
