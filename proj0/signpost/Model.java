 ...
 *  0.  Unnumbered, unlinked cells are in group -1.
 *
 *  On the board displayed to the user,cells in
 *  the same group indicate their grouping and sequence
 ...
 *  <i>head</i> of the group.
 *
 *  Each unnumbered square points to the head
...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        ArrayList<Integer> solNum = new ArrayList<Integer>();
        _solnNumToPlace = new Place[size() + 1];
        for (int i = 0; i < width(); i++) {
            for (int j = 0; j < height(); j++) {
                if (_solution[i][j] == 1) {
                    _board[i][j] = new Sq(i, j,
                            1, true, arrowDirection(i, j), -1);
                } else if (_solution[i][j] == size()) {
                    _board[i][j] = new Sq(i, j,
                            size(), true, arrowDirection(i, j), -1);
                } else {
                    _board[i][j] = new Sq(i, j,
                            0, false, arrowDirection(i, j), -1);
                }
                _allSquares.add(_board[i][j]);
                int k = _solution[i][j];
                _solnNumToPlace[k] = pl(i, j);
                solNum.add(k);
            }
        }
        for (int i = 0; i < size(); i++) {
            Collections.sort(solNum);
            if (solNum.get(i) == i) {
        ...
        for (Sq sq : _allSquares) {
            if (sq.successors() != null) {
                for (Place p : sq._successors) {
                    if (sq.connectable(get(p))) {
                        get(p)._predecessors.
                                add(pl(sq.x, sq.y));
                    }
                }
    ...
    Model(Model model) {
        ...
        for (int i = 0; i < width(); i++) {
            for (int j = 0; j < height(); j++) {
                this._board[i][j] = new Sq(model._board[i][j]);
                _board[i][j]._predecessors = new PlaceList();
                _board[i][j]._successors =
                        _allSuccessors[i][j][arrowDirection(i, j)];
                this._allSquares.add(_board[i][j]);
            }
        }
        for (int i = 0; i < width(); i++) {
            for (int j = 0; j < height(); j++) {
                Sq succ = _board[i][j].successor();
                Sq pred = _board[i][j].predecessor();
                Sq head = _board[i][j].head();
                _board[i][j]._successor = succ;
                _board[i][j]._predecessor = pred;
                _board[i][j]._head = head;
            }
        }
    }
    ...
    boolean autoconnect() {
        int count = 0;
        for (int i = 0; i < size(); i++) {
            Sq sq1 = _allSquares.get(i);
            if (sq1._sequenceNum != 0) {
                for (int j = 0; j < size(); j++) {
                    Sq sq2 = _allSquares.get(j);
                    if (sq2._sequenceNum != 0 && sq1.connectable(sq2)) {
                        sq1.connect(sq2);
                        count++;
                    }
                }
    ...
    final class Sq {
         ...
         *  + If neither S1 nor this square have sequence numbers,
         *      then it has to satisfy the condition that
        ...
        boolean connectable(Sq s1) {
            ...
            if (this.sequenceNum() == size()) {
                return false;
            }
            if (this.sequenceNum() != 0 && s1.sequenceNum() != 0) {
                return s1.sequenceNum() != 1 && this.sequenceNum()
                        == s1.sequenceNum() - 1 && s1.predecessor()
                        == null && this.successor()
                        == null;
            } else if (this.sequenceNum() == 0 && s1.sequenceNum()
                    == 0 && s1.predecessor() == null
                    && this.successor() == null) {
                if (this.sequenceNum() != size()) {
        ...
        boolean connect(Sq s1) {
            ...
            int s1gp = s1._group; int thisgp = _group;
            _unconnected -= 1;
            _successor = s1;
            s1._predecessor = this;

            if (_sequenceNum != 0 && s1._sequenceNum == 0) {
                int nextnum = sequenceNum() + 1;
                for (Sq sqthis = _successor;
                     sqthis != null; sqthis = sqthis._successor, nextnum++) {
                    sqthis._sequenceNum = nextnum;
                    sqthis._group = _group;
                }
                releaseGroup(s1gp);

            } else if (_sequenceNum == 0 && s1._sequenceNum != 0) {
                int prenum = s1.sequenceNum() - 1;
                for (Sq sqpred = s1._predecessor;
                     sqpred != null; sqpred = sqpred._predecessor, prenum--) {
                    sqpred._sequenceNum = prenum;
                    sqpred._group = s1._group;
                }
                releaseGroup(thisgp);

            } else if (_sequenceNum == 0
                    && s1._group != 0 && thisgp != 0) {
                _head._group = joinGroups(thisgp, s1gp);

            } else {
                _head._group = 0;
            }

            for (Sq hdsuc = _head._successor;
                 hdsuc != null; hdsuc = hdsuc._successor) {
                hdsuc._head = _head;
                hdsuc._group = _head._group;
            }

            return true;
        }

        /** help disconnect method, for saving space. */
        void disconnecthelper() {
            boolean fixed = true;
            if (!this.hasFixedNum()) {
                for (Sq pred = _predecessor; pred != null;
            ...
            if (fixed) {
                Sq lastpre = _predecessor;
                _sequenceNum = 0;
                if (lastpre != null) {
                    int neww = newGroup();
                    _group = neww;
                    for (Sq sq = lastpre; sq != null;
                         sq = sq._predecessor) {
                        sq._sequenceNum = 0;
                        sq._group = neww;
                    }
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                if (this._predecessor == null
                    ...
                    for (Sq nexsuc = next._successor;
                         nexsuc != null;
                         nexsuc = nexsuc._successor) {
                        nexsuc._group = next._group;
                    }
                }
            } else {
                disconnecthelper();
                boolean fixed2 = true;
                if (!next.hasFixedNum()) {
                    for (Sq nexsuc = next; nexsuc != null;
                        nexsuc = nexsuc._successor) {
                        if (nexsuc.hasFixedNum()) {
                            fixed2 = false;
                            break;
                        }
                    }
                ...
                if (fixed2) {
                    Sq nextsuc = next._successor;
                    next._sequenceNum = 0;
                    if (nextsuc != null) {
                        int neww = newGroup();
                        next._group = neww;
                        for (Sq sq = nextsuc; sq != null; sq = sq._successor) {
                            sq._sequenceNum = 0;
                            sq._group = neww;
                        }
                    } else {
