...
class PuzzleGenerator implements PuzzleSource {
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        ...
        PlaceList placelist = model.allSuccessors(
                Place.pl(start.x, start.y), dir);
        PlaceList connectab = new PlaceList();
        PlaceList copy = placelist;

        int i = 0;
        while (i < placelist.size()) {
            Place pl = copy.get(i);
            if (start.connectable(model.get(pl))) {
                connectab.add(pl);
            }
            i++;
        }
        if (connectab.size() == 1) {
            return model.get(connectab.get(0));

        } else if (start.sequenceNum() != 0) {
            for (Place pl: placelist) {
                if (start.connectable(model.get(pl))) {
                    Sq sq = model.get(pl);
                    int num = sq.sequenceNum();
                    if (num == start.sequenceNum() + 1) {
                        return sq;
                    }
                }
            }
    ...
    static Sq findUniquePredecessor(Model model,
                                    Sq end) {

        int check = 0;
        int siz = 0;
        int uniNotsq = 0;
        PlaceList numberedlist = new PlaceList();
        PlaceList uniqueplace = new PlaceList();


        PlaceList pre = end.predecessors();
        PlaceList uniqueplace2 = new PlaceList();
        PlaceList uniqueplace3 = new PlaceList();

        for (Place pl: pre) {
            if (model.get(pl).sequenceNum() == 0
                    || model.get(pl).connectable(end)) {
                uniNotsq += 1;
                uniqueplace2.add(pl);
            }
            if (model.get(pl).connectable(end)) {
                siz += 1;
                uniqueplace3.add(pl);
            }
        }
        if (uniNotsq == 1) {
            return model.get(uniqueplace2.get(0));
        }

        if (siz == 1) {
            return model.get(uniqueplace3.get(0));
        }
        if (pre != null) {
            for (Place pl : pre) {
                if (model.get(pl).sequenceNum() != 0) {
        ...
        if (check == 1 && numberedlist.contains(uniqueplace.get(0))) {
            return model.get(uniqueplace.get(0));
        }
