...
public class ArrayHeap<T> {
    ...
    private int min(int index1, int index2) {
        // TODO
        return Objects.requireNonNull(getNode(index1)).priority() > Objects.requireNonNull(getNode(index2)).priority() ? index1 : index2;
    }
    ...
    private void bubbleUp(int index) {
        ...
        Node par = getNode(getParentOf(index)) ;
    ...
    public T removeMin() {
        if (getNode(1) != null) {
            T top = Objects.requireNonNull(getNode(1)).item();
            swap(1, size());
            removeNode(size());
            bubbleDown(1);
            return top;
